...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # 连接信号到槽
        self.ui.pushButton.clicked.connect(self.openFileNameWidget)
        self.ui.pushButton_2.clicked.connect(self.on_upload_clicked)
        self.setAcceptDrops(True)  # 允许窗口接受拖拽

    def openFileNameWidget(self):
        # 使用QFileWidget打开文件选择对话框
        options = QDialog.Options()
        fileName, _ = QDialog.getOpenFileName(self, "选择文件", "", "All Files (*);;Python Files (*.py)",
                                                  options=options)
        if fileName:
